...
anti <- st_read(here::here("app/biodiversity/anthirrinum.shp"), quiet = TRUE)
ac <- st_transform(anti, 4326)
# 


# read the data 
gps <- readRDS(here::here("app/biodiversity/cg_filtered.rds"))
  ...
  navbarPage(
    ...
    tabPanel(
      ...
      absolutePanel(
        ...
        top = 60, right = 20, left = "auto", bottom = "auto",
  ...
  output$mymap <- renderLeaflet({
    ...
    leaflet(data = filteredData()) %>%
      ...
      addWMSTiles('http://www.ideandalucia.es/wms/mta10v_2007?',
                  ...
                  group = 'topo2007') %>% 
      addPolygons(data = ac, 
                  group = "Antirrhinum charidemi", 
                  color = "black",
                  fillOpacity = 0.8
                  ) %>%
      addLayersControl(overlayGroups = c("pts","density", "Antirrhinum charidemi"),
